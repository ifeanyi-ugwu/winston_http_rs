...
impl Proxy for HttpTransport {
    fn proxy(&self, _target: &dyn Proxy) -> Result<usize, String> {
...
mod tests {
    ...
    fn test_single_log_http_send() {
        ...
        transport.log(timestamp().transform(log, None).unwrap());
    ...
    fn test_batched_logs_http_send() {
        ...
        transport.log(timestamp().transform(log1, None).unwrap());
        transport.log(timestamp().transform(log2, None).unwrap());
        ...
        transport.log(timestamp().transform(log3, None).unwrap());
